...
local response = require "ext.github.doujiang24.lua-resty-kafka.response"
local request = require "ext.github.doujiang24.lua-resty-kafka.request"
local broker = require "ext.github.doujiang24.lua-resty-kafka.broker"
local client = require "ext.github.doujiang24.lua-resty-kafka.client"
local Errors = require "ext.github.doujiang24.lua-resty-kafka.errors"
local sendbuffer = require "ext.github.doujiang24.lua-resty-kafka.sendbuffer"
local ringbuffer = require "ext.github.doujiang24.lua-resty-kafka.ringbuffer"
...
local function _send(self, broker_conf, topic_partitions)
    ...
    -- ldeng: workaroud
    local host = broker_conf.host
    for _, b in ipairs(self.broker_list) do
        if b.name and host == b.name then
            host = b.host
            break
        end
    end
    ...
    local bk, err = broker:new(host, broker_conf.port, self.socket_config)
    -- ldeng: workaroud end
    if bk then
...
function _M.new(self, broker_list, producer_config, cluster_name)
    ...
    local p = setmetatable({
        broker_list = broker_list, -- ldeng: workaroud
        client = cli,
        correlation_id = 1,
